...
def parse_inventory(inventory_text: str) -> tuple[date, int, int, int, int, list[str]]:
    ...
    repositories = [
        ...
        for match in INVENTORY_REPOSITORY_PATTERN.finditer(inventory_text)
    ]
    return (
        observed,
...
def fetch_live_public_repositories() -> dict[str, dict[str, object]]:
    ...
    while True:
        request = Request(
            GITHUB_PUBLIC_REPOSITORIES_URL.format(page=page),
...
def main() -> None:
    ...
    if not isinstance(authority, dict) or authority.get("owner") != "GoreeCloud":
    ...
    for repository, branch_name in overrides.items():
        if not isinstance(branch_name, str) or not branch_name.strip() or branch_name == "main":
