...
The canonical `GoreeCloud/goreecloud-glaze-ui` repository identifies `GLAZE UI V1.4 / 1.4.0` as the current Stable consumer target. Contract v0.3 requires applicable consumers to declare `compatibility.glaze_ui_required: "1.4.0"`. V1.3 / `1.3.0` remains the direct historical rollback baseline in the Glaze lifecycle; a current baseline declaration does not establish downstream conformance by itself.
